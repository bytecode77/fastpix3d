...
//#define OBJL_CONSOLE_OUTPUT // bytecode77: Do not output on the console
...
namespace objl
{
	...
	struct Material
	{
		Material()
		{
			...
			Ns = 0.0f;
			Ni = 0.0f;
			d = 1.0f; // bytecode77: Changed default value to 1.0
			illum = 0;
		}
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "mtllib")
				{
					...
					algorithm::split(Path, temp, "\\"); // bytecode77: Changed "/" to "\"
					...
					if (temp.size() != 1)
					{
						...
						{
							pathtomat += temp[i] + "\\"; // bytecode77: Changed "/" to "\"
						}
					}
	...
	private:
			...
			const std::vector<Vertex>& iVerts)
		{
			...
			while (true)
			{
				...
				for (int i = 0; i < int(tVerts.size()); i++)
				{
					...
					float angle = math::AngleBetweenV3(pPrev.Position - pCur.Position, pNext.Position - pCur.Position) * (180 / 3.1415927f);
		...
		bool LoadMaterials(std::string path)
		{
			...
			// bytecode77: Allow other common file extensions, such as .lib
