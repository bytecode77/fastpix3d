...
Mesh* PrimitiveFactory::Plane(int32 xSegments, int32 ySegments)
{
	...
	for (int32 y = 0, index = 0; y <= ySegments; y++)
	{
		for (int32 x = 0; x <= xSegments; x++)
		{
			...
			surface->SetVertex(index++, vfloat3(xf, 0, yf), vfloat3(0, 1, 0), vfloat2(-v, u));
		}
	}
		...
		for (int32 y = 0, index = 0; y <= sideSegmentsY; y++)
		{
			for (int32 x = 0; x <= sideSegmentsX; x++)
			{
				...
				switch (side)
				{
					case 0:
						// Top
						surface->SetVertex(index++, vfloat3(xf, .5f, yf), vfloat3(0, 1, 0), vfloat2(-v, u));
						break;
					case 1:
						// Bottom
						surface->SetVertex(index++, vfloat3(yf, -.5f, xf), vfloat3(0, -1, 0), vfloat2(u, v));
						break;
					case 2:
						// Left
						surface->SetVertex(index++, vfloat3(-.5f, xf, yf), vfloat3(-1, 0, 0), vfloat2(v, -u));
						break;
					case 3:
						// Right
						surface->SetVertex(index++, vfloat3(.5f, yf, xf), vfloat3(1, 0, 0), vfloat2(u, v));
						break;
					case 4:
						// Back
						surface->SetVertex(index++, vfloat3(xf, yf, -.5f), vfloat3(0, 0, -1), vfloat2(u, v));
						break;
					case 5:
						// Front
						surface->SetVertex(index++, vfloat3(yf, xf, .5f), vfloat3(0, 0, 1), vfloat2(v, -u));
						break;
				}
			}
		}
...
Mesh* PrimitiveFactory::Cylinder(int32 segments, float topRadius, float bottomRadius)
{
	...
	top->SetVertex(0, vfloat3(0, .5f, 0), vfloat3(0, 1, 0), vfloat2(.5f, .5f));
	bottom->SetVertex(0, vfloat3(0, -.5f, 0), vfloat3(0, -1, 0), vfloat2(.5f, .5f));
	...
	for (int32 i = 0; i < segments; i++)
	{
		...
		top->SetVertex(i + 1, vfloat3(c * .5f * topRadius, .5f, s * .5f * topRadius), vfloat3(0, 1, 0), vfloat2(s * .5f + .5f, c * .5f + .5f));
		bottom->SetVertex(i + 1, vfloat3(c * .5f * bottomRadius, -.5f, s * .5f * bottomRadius), vfloat3(0, -1, 0), vfloat2(s * .5f + .5f, .5f - c * .5f));
	...
	for (int32 i = 0; i <= segments; i++)
	{
		...
		side->SetVertex(i, vfloat3(c * .5f * topRadius, .5f, s * .5f * topRadius), vfloat3(c, 0, s), vfloat2((float)i / segments + .5f, 0));
		side->SetVertex(i + segments + 1, vfloat3(c * .5f * bottomRadius, -.5f, s * .5f * bottomRadius), vfloat3(c, 0, s), vfloat2((float)i / segments + .5f, 1));
	}
...
Mesh* PrimitiveFactory::Tube(int32 segments, float topRadius, float topInnerRadius, float bottomRadius, float bottomInnerRadius)
{
	...
	for (int32 i = 0; i < segments; i++)
	{
		...
		top->SetVertex(i, vfloat3(c * .5f * topRadius, .5f, s * .5f * topRadius), vfloat3(0, 1, 0), vfloat2(s * .5f + .5f, c * .5f + .5f));
		top->SetVertex(i + segments, vfloat3(c * .5f * topInnerRadius, .5f, s * .5f * topInnerRadius), vfloat3(0, 1, 0), vfloat2(s * .5f * topInnerRadius / topRadius + .5f, c * .5f * topInnerRadius / topRadius + .5f));

		bottom->SetVertex(i, vfloat3(c * .5f * bottomRadius, -.5f, s * .5f * bottomRadius), vfloat3(0, -1, 0), vfloat2(s * .5f + .5f, .5f - c * .5f));
		bottom->SetVertex(i + segments, vfloat3(c * .5f * bottomInnerRadius, -.5f, s * .5f * bottomInnerRadius), vfloat3(0, -1, 0), vfloat2(s * .5f * bottomInnerRadius / bottomRadius + .5f, .5f - c * .5f * bottomInnerRadius / bottomRadius));
	...
	for (int32 i = 0; i <= segments; i++)
	{
		...
		inside->SetVertex(i, vfloat3(c * .5f * topInnerRadius, .5f, s * .5f * topInnerRadius), vfloat3(-c, 0, -s), vfloat2((float)i / segments + .5f, 0));
		inside->SetVertex(i + segments + 1, vfloat3(c * .5f * bottomInnerRadius, -.5f, s * .5f * bottomInnerRadius), vfloat3(-c, 0, -s), vfloat2((float)i / segments + .5f, 1));
	}
...
Mesh* PrimitiveFactory::Cone(int32 segments)
{
	...
	for (int32 i = 0; i < segments; i++)
	{
		...
		bottom->SetVertex(i + 1, vfloat3(c * .5f, -.5f, s * .5f), vfloat3(0, -1, 0), vfloat2(s * .5f + .5f, .5f - c * .5f));
		bottom->SetTriangle(i, 0, i + 1, (i + 1) % segments + 1);
	}
	...
	for (int32 i = 0; i <= segments; i++)
	{
		...
		side->SetVertex(i, vfloat3(0, .5f, 0), vfloat3(c, .5f, s).Normalize(), vfloat2((i + .5f) / segments + .5f, 0));
		side->SetVertex(i + segments + 1, vfloat3(c * .5f, -.5f, s * .5f), vfloat3(c, .5f, s).Normalize(), vfloat2((float)i / segments + .5f, 1));
	}
...
Mesh* PrimitiveFactory::Sphere(int32 xSegments, int32 ySegments)
{
	...
	for (int32 y = 0, index = 0; y <= ySegments; y++)
	{
		...
		for (int32 x = 0; x <= xSegments; x++)
		{
			...
			surface->SetVertex(index++, vfloat3(cx * cy * .5f, sy * .5f, sx * cy * .5f), vfloat3(cx * cy, sy, sx * cy), vfloat2((float)x / xSegments, (float)y / ySegments));
		}
	}
...
Mesh* PrimitiveFactory::Torus(int32 xSegments, int32 ySegments, float innerRadius)
{
	...
	for (int32 y = 0, index = 0; y <= ySegments; y++)
	{
		...
		for (int32 x = 0; x <= xSegments; x++)
		{
			...
			surface->SetVertex(index++, vfloat3(cx * outerRadius + cx * cy * innerRadius, sy * innerRadius, sx * outerRadius + sx * cy * innerRadius), vfloat3(cx * cy, sy, sx * cy), vfloat2((float)x / xSegments + .5f, .25f - (float)y / ySegments));
		}
	}
