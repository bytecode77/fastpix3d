...
FreeLook::FreeLook() : FreeLook(vfloat3())
{
}
FreeLook::FreeLook(const vfloat3 &position) : FreeLook(position, vfloat2())
{
}
FreeLook::FreeLook(const vfloat3 &position, const vfloat2 &rotation) : FreeLook(position, rotation, 1)
{
}
...
void FreeLook::Update()
{
	...
	Matrix4 walkMatrix =
		...
		Matrix4::RotateX(_Rotation.Y) *
		Matrix4::RotateY(_Rotation.X);

	_PositionVelocity = (_PositionVelocity + walkMatrix * vfloat3()) * .8f;
	_Position += _PositionVelocity;

	_RotationVelocity = (_RotationVelocity - vfloat2(-Input::MouseSpeed.X * .1f, -Input::MouseSpeed.Y * .1f)) * .75f;
