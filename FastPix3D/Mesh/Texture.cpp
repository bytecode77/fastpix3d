...
Texture::Texture(int32 width, int32 height)
{
	...
	GetMipmapParameters(_Width, _Height, _MipLevels, _BufferSize);
	_Buffer = new Color[_BufferSize];
...
Texture::Texture(const Texture &other) :
	_FileChecksum(other._FileChecksum),
	_Type(other._Type),
	_Width(other._Width),
	_Height(other._Height),
	_WidthExponent(other._WidthExponent),
	_MipLevels(other._MipLevels),
	_HasTransparencyKey(other._HasTransparencyKey)
{
	if (other._Path)
	{
		char *pathCopy = new char[lstrlenA(other._Path) + 1];
		lstrcpyA(pathCopy, other._Path);
		_Path = pathCopy;
	}
	else
	{
	...
	_Buffer = new Color[_BufferSize];
	memcpy(_Buffer, other._Buffer, _BufferSize * sizeof(Color));
	ComputeMipMapOffsets();
}
Texture::~Texture()
{
...
Texture* Texture::FromMemory(const void *buffer, int32 size)
{
	...
	Texture *existingTexture = GetExistingTexture(checksum);
	if (existingTexture)
	{
		return existingTexture;
	}
...
Texture* Texture::FromBitmap(const Bitmap *bitmap)
{
	...
	{
		memcpy(texture->_Buffer, bitmap->Pixels, texture->_Width * texture->_Height * sizeof(Color));
	}
	else
	{
...
Texture* Texture::GetExistingTexture(const char *path)
{
	...
	{
		if (texture->_Path && !lstrcmpiA(texture->_Path, path))
		{
...
Texture* Texture::GetExistingTexture(uint32 checksum)
{
	for (Texture *texture : AllTextures)
	{
		if (texture->_FileChecksum && texture->_FileChecksum == checksum)
		{
			// Texture was already loaded.
			return texture;
		}
...
void Texture::ComputeMipMapOffsets()
{
